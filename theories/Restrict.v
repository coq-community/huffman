...
Variable m : list A.
(* 
   Restrict the code putting only codes of element in the frequency
   list
   *)
...
Qed.
(* 
   Doing the restriction do not change the codes
   *)
...
Qed.
(* 
   The leaves of the build tree from the restrict code is the keys
...
Theorem restrict_code_pbbuild :
 ...
 in_alphabet m c ->
 m <> nil ->
 permutation (map (fst (B:=_)) (frequency_list eqA_dec m))
